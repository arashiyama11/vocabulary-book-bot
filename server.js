...
function sendAllLog(msg,addTopURL) {
  client.channels.cache.filter(ch => ch.name === "単語帳log").forEach(async ch => {
    ...
    if(addTopURL)msg.embeds[0].setURL(msgs.map((m)=>m)[0].url)
    ch.send(msg)
  })
}
...
client.on("channelDelete",(channel)=>{
  const guildData=data.find(d=>d.guildid===channel.guild.id)
  if(guildData===undefined)return
  guildData.data=guildData.data.filter(d=>d.channelid!==channel.id)
...
client.on("messageCreate", async message => {
  ...
  if (message.channel.name === "単語帳ターミナル") {
    ...
    if ((message.content.startsWith("！テスト開始") || message.content.startsWith("!start")) && !thisGuildTestData.testing) {
      ...
      const firstMsg=await questionsChannel.messages.fetch({ after: "0", limit: 1 })
      if(firstMsg.author!==client.user.username)return message.channel.send("「!mkch」コマンドで作成したチャンネルでしかテストすることが出来ません。")
      if (isNaN(line[2] - 0)) {
    ...
    if (thisGuildTestData.testing && (((message.author.username === client.user.username && message.content === "テストを開始します") || message.author.id === thisGuildTestData.user))) {
      ...
      if (thisGuildTestData.testing &&
        ...
        let ransu = Math.floor(Math.random() * thisGuildTestData.questions.length);
        while (thisGuildTestData.tested.includes(ransu)) {
          ransu = Math.floor(Math.random() * thisGuildTestData.questions.length);
        }
        ...
        if (thisguild.data.find(d => d.channelid === thisGuildTestData.channel.id) === undefined) {
          thisguild.data.push({
            channelid: thisGuildTestData.channel.id,
            data: []
          });
        }
        let thisdata = thisguild.data.find(d => d.channelid == thisGuildTestData.channel.id).data;
        ...
        thisGuildTestData.channel.messages
          ...
          .then(mesg => {
            ...
            for (let a = 0; a < SoFA.length; a++) {
              let per =
                Math.round((thisdata[a].s * 10000) / (thisdata[a].s + thisdata[a].f)) / 100;
              let b = a + 1;
